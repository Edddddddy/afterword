...
- Generated notes are written to `OUTPUT_DIR` (default `~/Desktop`); temp session data lives in `SESSION_DIR` (default `~/.afterword/sessions`). The backend creates both on startup.
